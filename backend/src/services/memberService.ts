...
import { SERVICE, Error400, isDomainExcluded } from '@gitmesh/common'
import { LoggerBase } from '@gitmesh/logging'
import { WorkflowIdReusePolicy } from '@gitmesh/temporal'
import {
  FeatureFlag,
  IOrganization,
  ISearchSyncOptions,
...
import { UnifiedQueryStrategy } from '../utils/queryStrategyHelper'
...
export default class MemberService extends LoggerBase {
  ...
  async upsert(
    ...
    fireGitmeshWebhooks: boolean = true,
      ...
      if (existing) {
        ...
        telemetryTrack(
          'Member created',
          {
            ...
            sample: record.attributes.sample?.gitmesh,
  ...
  async queryV2(data) {
    ...
    if (result.count !== result.rows.length) {
      logger.warn(
        { originalCount: result.count, actualRows: result.rows.length }, 
