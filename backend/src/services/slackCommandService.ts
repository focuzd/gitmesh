...
import { validateUUID as uuidValidate } from '@gitmesh/common'
...
export default class SlackCommandService {
  ...
  public async processCommand(
    ...
    if (command === '/gitmesh-test' && !IS_DEV_ENV) {
      ...
      return SlackCommandService.buildSlackMessage(
        Section().text('Received /gitmesh-test command in non-dev environment! Ignoring!'),
      )
    }

    if (command === '/gitmesh-staging' && !IS_STAGING_ENV) {
      this.options.log.error(
        'Received /gitmesh-staging command in non-staging environment! Ignoring!',
      )
      return SlackCommandService.buildSlackMessage(
        Section().text('Received /gitmesh-staging command in non-staging environment! Ignoring!'),
      )
    }

    if (command === '/gitmesh' && !IS_PROD_ENV) {
      this.options.log.error('Received /gitmesh command in non-prod environment! Ignoring!')
      return SlackCommandService.buildSlackMessage(
        Section().text('Received /gitmesh command in non-prod environment! Ignoring!'),
      )
    }
    ...
    if (!commandDefinition) {
      return SlackCommandService.buildSlackMessage(
        Section().text(`Command not found! Try \`/gitmesh help\`.`),
      )
    }
