    ...
    <div class="flex-grow">
      <div v-if="(activity.title || displayTitleBody) || activity.body">
        ...
        <div v-if="activity.display?.default && displayTitleBody">
          <div
            class="first-letter:uppercase font-semibold text-gray-200 text-sm mb-1"
            v-html="
              contentRenderEmojis(
      ...
      <div v-else-if="activity.display?.default">
        <div
          class="first-letter:uppercase font-semibold text-gray-200 text-sm"
          v-html="
            contentRenderEmojis(
              $sanitize($marked(activity.display.default)),
            )
...
export default {
  ...
  computed: {
    platformConfig() {
      return GitmeshIntegrations.getConfig(
