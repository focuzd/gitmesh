  ...
  <div class="activity-timeline bg-black text-white px-0 sm:px-4">
    ...
    <div>
      <el-timeline>
          ...
          >
            <div class="-mt-1.5">
              <app-member-display-name
                ...
                custom-class="block text-zinc-100 font-medium"
                with-link
                class="bl"
              />
              <div
            ...
            <template #dot>
              <span
                class="btn btn--circle cursor-auto p-2 bg-zinc-800 border border-zinc-700"
              ...
              >
                ...
                <i
                  v-else
                  class="ri-radar-line text-base text-zinc-400"
                />
              </span>
            </template>
          </el-timeline-item>
        ...
        >
          <div
            class="ri-list-check-2 text-3xl text-zinc-200 mr-4 h-10 flex items-center"
          />
          <p
            class="text-xs leading-5 text-center italic text-zinc-400"
          >
...
const activeIntegrations = computed(() => {
  ...
  return Object.keys(activeIntegrationList).map((i) => ({
    ...
    label: GitmeshIntegrations.getConfig(i).name,
  }));
});
...
const platformDetails = (p) => GitmeshIntegrations.getConfig(p);

const debouncedQueryChange = debounce(async () => {
  await fetchActivities();
}, 300);

const getPlatformDetails = (p) => GitmeshIntegrations.getConfig(p);
