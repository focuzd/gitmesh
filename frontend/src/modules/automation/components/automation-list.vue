<template>
  <div class="pt-6">
      ...
      <div class="flex items-center">
        <div class="flex text-xs text-zinc-400">
          <div
            ...
            :key="option.value"
            class="h-8 border-solid border-zinc-700 border-r border-y first:border-l flex items-center
       justify-center transition hover:bg-zinc-800 cursor-pointer first:rounded-l-md last:rounded-r-md px-3 bg"
            :class="filter.type === option.value ? 'font-medium bg-zinc-800 text-zinc-100' : 'bg-zinc-900'"
            @click="changeAutomationFilter({ type: option.value })"
          >
            {{ option.label }}
          </div>
        </div>
        <p class="pl-4 text-xs leading-5 text-zinc-200">
        ...
        >
          ...
          >
            <div
              ...
              :class="{
                'hover:bg-zinc-900 !cursor-default': !automationType.canCreate(store),
            ...
            >
              <div class="flex">
                ...
                <div class="pl-2">
                  ...
                  <p class="text-2xs leading-4.5 text-zinc-200 text-left break-normal">
    ...
    <app-empty-state-cta
      ...
      description="Automate actions or Slack alerts for new activity or contacts."
    />
...
.empty-list-icon {
  font-size: 160px;
  @apply leading-none text-zinc-700;
}
</style>
