...
// UPDATED: Set to 7 to match UI component logic for Error/Red status
export const ERROR_BANNER_WORKING_DAYS_DISPLAY = 7;
...
export default {
  ...
  getters: {
    ...
    list: (state) => Object.keys(state.byId).reduce((acc, key) => {
      const integrationJsonData = GitmeshIntegrations.getConfig(
  ...
  actions: {
    ...
    async doGithubConnect({ commit, dispatch }, { code, installId, setupAction }) {
      try {
        commit('CREATE_STARTED');
        const integration = await IntegrationService.githubConnect(code, installId, setupAction);
    ...
    async doLinkedinOnboard({ commit }, organizationId) {
      try {
        ...
        const integration = await IntegrationService.linkedinOnboard(organizationId);
    ...
    async doDevtoConnect({ commit }, { users, organizations, apiKey }) {
      try {
        commit('CREATE_STARTED');
        const integration = await IntegrationService.devtoConnect(users, organizations, apiKey);
        commit('CREATE_SUCCESS', integration);
    ...
    async doHackerNewsConnect({ commit }, { keywords, urls }) {
      try {
        commit('CREATE_STARTED');
        const integration = await IntegrationService.hackerNewsConnect(keywords, urls);
        commit('CREATE_SUCCESS', integration);
    ...
    async doGitConnect({ commit }, { remotes, isUpdate }) {
      try {
        ...
        Message.success(
          'The first activities will show up in a couple of seconds. <br /> <br /> This process might take a few minutes to finish, depending on the amount of data.',
          { title: `Git integration ${isUpdate ? 'updated' : 'created'} successfully` },
        );
        router.push('/integrations');
      } catch (error) {
        Errors.handle(error);
    ...
    async doDiscourseConnect({ commit }, { forumHostname, apiKey, webhookSecret, isUpdate }) {
      try {
        commit('CREATE_STARTED');
        const integration = await IntegrationService.discourseConnect(forumHostname, apiKey, webhookSecret);
        commit('CREATE_SUCCESS', integration);

        Message.success(
          'The first activities will show up in a couple of seconds. <br /> <br /> This process might take a few minutes to finish, depending on the amount of data.',
          { title: `Discourse integration ${isUpdate ? 'updated' : 'created'} successfully` },
        );
        router.push('/integrations');
      } catch (error) {
        Errors.handle(error);
    ...
    async doGroupsioConnect({ commit }, { email, token, groupNames, isUpdate }) {
      try {
        commit('CREATE_STARTED');
        const integration = await IntegrationService.groupsioConnect(email, token, groupNames);
        commit('CREATE_SUCCESS', integration);

        Message.success(
          'The first activities will show up in a couple of seconds. <br /> <br /> This process might take a few minutes to finish, depending on the amount of data.',
          { title: `groups.io integration ${isUpdate ? 'updated' : 'created'} successfully` },
        );
        
        router.push({
          name: 'integration',
          params: { id: integration.segmentId },
        });
      } catch (error) {
        Errors.handle(error);
