<template>
  <div class="grid gap-x-12 grid-cols-4">
    <div v-if="showHeader">
      ...
      <p class="text-zinc-200 text-2xs leading-normal mt-1">
    ...
    >
      <div
        ...
        :key="key"
        class="border-b border-zinc-700 last:border-none"
      >
...
const identitiesForm = reactive({
  devto: {
    ...
    urlPrefix: 'dev.to/',
    imgContainerClass:
      'h-8 w-8 rounded flex items-center justify-center text-base bg-zinc-800 border border-zinc-700',
  },
  discord: {
    enabled:
  ...
  slack: {
    ...
    urlPrefix: 'slack.com/',
    imgContainerClass:
      'h-8 w-8 rounded flex items-center justify-center text-base btn--slack cursor-auto hover:cursor-auto bg-zinc-900 border border-zinc-700',
  },
  twitter: {
    enabled:
...
function findPlatform(platform) {
  return GitmeshIntegrations.getConfig(platform);
}
...
const removeUsername = (platform, index) => {
  ...
  if (!model.value.username[platform]?.length) {
    ...
    model.value.attributes = {
      ...
      url: {
        ...props.modelValue.attributes?.url,
        [platform]: GitmeshIntegrations.getConfig(platform)?.url({ username: model.value.username[platform][0], attributes: model.value.attributes }),
      },
    };
  }
};
